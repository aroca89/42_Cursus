...
/*   Updated: 2023/09/12 20:48:15 by aroca-pa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void check_points(t_map *map)
{
    ...
    {  
        ...
        {  
            ...
            if (pixel == 'E')
            {
                map->exit_position_row = x; // Asignar la fila actual
                map->exit_position_col = y;
                map->exit++;
            }
            if (pixel == 'C')
                map->collectibles++;
...
int is_map_passable(t_map *map)
{
    ...
    while (y < map->rows) // Bucle externo para recorrer las filas.
    {
